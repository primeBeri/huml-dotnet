...
Two convenience instances are provided for common scenarios: `HumlOptions.Default` and `HumlOptions.AutoDetect`.
...
|----------|-------------|---------------|------------------------|------------------|-------------------|
| `HumlOptions.Default` | V0_2 | Options | Throw | Multiline | 512 |
| `HumlOptions.AutoDetect` | V0_2 | Header | Throw | Multiline | 512 |

`HumlOptions.AutoDetect` reads the `%HUML vX.Y.Z` header from the document to determine the spec version.
If no header is present, it falls back to `V0_2`.
...
- Passing `null` for `options` in any `Huml.*` method is equivalent to passing `HumlOptions.Default`.
